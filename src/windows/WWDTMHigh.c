...
static uint16_t patterns[1][GRID_SIZE];

static void win_init(int id)
{
    memcpy(patterns, dec_patterns + 89, sizeof(patterns));
...
static void win_event(int id,int sample)
{
    ...
    if(sample == 17)
    {
        copy_id = 0; mov_bump(&motion, 0, -motion.height / 10);
    } else if(sample == 18) {
        copy_id = 0; mov_bump(&motion, 0, -motion.height / 13);
    } else if(sample == 49) {
        copy_id = 0; mov_bump(&motion, 0, -motion.height / 16);
    } else if(sample == 50) {
        copy_id = 0; mov_bump(&motion, 0, -motion.height / 19);
    }
