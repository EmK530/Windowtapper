...
static uint16_t patterns[8][GRID_SIZE];

static void win_init(int id)
{
    memcpy(patterns, dec_patterns + 81, sizeof(patterns));
...
static void win_event(int id,int sample)
{
    ...
    if(sample >= 687 && sample <= 702)
    {
        copy_id = (sample-687)%4; mov_bump(&motion, 0, -motion.height / (5+((copy_id+1)%4)));
    } else if(sample >= 711 && sample <= 718)
    {
        copy_id = 4+((sample-711)%4); mov_bump(&motion, 0, -motion.height / (5+((copy_id+1)%4)));
    } else if(sample == 100)
    {
        pattern_frames = 0;
    }
...
static void win_update(int id, double dt)
{
    ...
    mov_update(&motion, windows[id].hwnd, 320, dt);
}
