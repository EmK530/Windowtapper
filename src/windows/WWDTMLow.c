...
{
    memcpy(patterns, dec_patterns + 90, sizeof(patterns));
...
static void win_event(int id,int sample)
{
    ...
    if(sample == 2)
    {
        copy_id = 0; mov_bump(&motion, -motion.height / 14, 0);
    } else if(sample == 7)
    {
        copy_id = 1; mov_bump(&motion, motion.height / 14, 0);
    }
