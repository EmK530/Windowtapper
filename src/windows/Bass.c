...
{
    memcpy(patterns, dec_patterns + 4, sizeof(patterns));
...
static void win_event(int id,int sample)
{
    ...
    {
        copy_id = 0; mov_bump(&motion, 0, -motion.height / 8);
    } else if(sample == 96)
    {
        copy_id = 1; mov_bump(&motion, 0, -motion.height / 5);
    } else if(sample == 97 || sample == 98)
    {
        copy_id = 2; mov_bump(&motion, 0, -motion.height / 7);
    } else if(sample == 99)
    {
        copy_id = 3; mov_bump(&motion, 0, -motion.height / 6);
    }
