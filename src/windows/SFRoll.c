...
static uint16_t patterns[2][GRID_SIZE];

static void win_init(int id)
{
    memcpy(patterns, dec_patterns + 78, sizeof(patterns));
...
{
    int copy_id = -1;
    int tgt_frames = MAX_FRAMES;

    if(sample == 9)
    {
        copy_id = 0; mov_bump(&motion, 0, motion.height / 12);
        tgt_frames = 6;
    } else if(sample == 4 || sample == 5 || sample == 22)
    {
        copy_id = 1; mov_bump(&motion, 0, -motion.height / 16);
    } else if(sample == 6)
    {
        copy_id = 1; mov_bump(&motion, 0, -motion.height / 16);
        tgt_frames = 4;
