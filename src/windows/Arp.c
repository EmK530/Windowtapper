...
static const uint8_t indexes[40] = {0,1,0,1,0,1,0,1,0,2,1,3,0,2,1,3,0,2,1,3,0,2,1,3,0,2,1,3,0,2,1,3,0,2,1,3,0,2,1,3};
static uint16_t patterns[4][GRID_SIZE];

static void win_init(int id)
{
    memcpy(patterns, dec_patterns + 0, sizeof(patterns));
...
static void win_event(int id,int sample)
{
    ...
    if(sample >= 458 && sample <= 662)
    {
        copy_id = indexes[(sample-458)%40]; move_id = copy_id%2;
    }
